...
PKGNAME=lilosetup
MODULES_DIR=lilosetup_modules
VER=$(grep 'application_version =' src/$PKGNAME.py | head -n 1 | sed "s/.*'\(.*\)'/\1/")
...
install -d -m 755 $DESTDIR/usr/share/$PKGNAME/$MODULES_DIR

for f in src/$MODULES_DIR/*.py; do
	install -D -m 644 $f $DESTDIR/usr/share/$PKGNAME/$MODULES_DIR/$(basename $f)
done

install -m 755 src/$PKGNAME.py $DESTDIR/usr/sbin/$PKGNAME.py
install -m 644 src/$PKGNAME.glade $DESTDIR/usr/share/$PKGNAME
...
install -m 644 icons/$PKGNAME-24.png $DESTDIR/usr/share/icons/hicolor/24x24/apps/$PKGNAME.png
install -m 644 icons/$PKGNAME-64.png $DESTDIR/usr/share/icons/hicolor/64x64/apps/$PKGNAME.png
...
install -m 644 src/$PKGNAME.png $DESTDIR/usr/share/$PKGNAME/
